...
use std::{convert::TryFrom, io::Write, num::ParseIntError, ops::Deref};
...
impl From<[u32; 2]> for Uint64 {
    ...
    fn from(slice: [u32; 2]) -> Self {
        u64::from_bits(slice[0], slice[1]).into()
    }
}
...
impl TryFrom<&str> for Uint64 {
    ...
    fn try_from(hex: &str) -> Result<Self, Self::Error> {
        <[u8; Self::BYTES]>::from_hex(hex)?;
        let int = u64::from_str_radix(hex, 16).unwrap();
        Ok(Self(int))
    }
}
...
pub trait AsUint64 {
    ...
    fn to_hex(&self) -> String {
        format!("{:X}", self.as_uint64())
    }

    /// Generate `u64` from a utf8 string.
    /// Deterministic u64 value for the given utf8 string.
    #[inline]
    fn from_utf8_str(source: &str) -> u64 {
        let hash = Sha3_256::digest(source.as_bytes());
        let mut source_hash = [0_u8; 8];
        source_hash.copy_from_slice(&hash[..8]);

        u64::from_le_bytes(source_hash)
    }

    /// Creates a `u64` from a hex string.
    #[inline]
    fn from_hex(hex: &str) -> Result<u64, ParseIntError> {
    ...
    /// Creates a `u64` from the given low and high bits.
    ///
    #[inline]
    fn from_bits(lower: u32, higher: u32) -> u64 {
        let mut buf = [0x0; Uint64::BYTES];
        buf[..4].as_mut().write(&higher.to_be_bytes()).expect("higher write error");
        buf[4..].as_mut().write(&lower.to_be_bytes()).expect("lower write error");
        u64::from_be_bytes(buf)
    }

    #[inline]
    fn get_bits(num: u64) -> [u32; 2] {
...
mod tests {
    ...
    static HEX_TEST_VECTORS: &[TestVector] = &[
        TestVector { str: "0000000000000000", dto: [0, 0] }, // "0"
        TestVector { str: "000000000000A1B2", dto: [0xa1b2, 0] }, // "(0, 8)"
        TestVector { str: "0000000012345678", dto: [0x12345678, 0] }, // "8"
        TestVector { str: "0000ABCD12345678", dto: [0x12345678, 0xabcd] }, //"(8, 16)"
        TestVector { str: "1234567890ABCDEF", dto: [0x90abcdef, 0x12345678] }, // "16"
        TestVector { str: "FFFFFFFFFFFFFFFF", dto: [0xffffffff, 0xffffffff] }, // "16 (max value)"
    ];
    ...
    #[test]
    fn test_should_be_created_from_utf8() {
        let value = u64::from_utf8_str("VAQXZH7TDE6EMG7KIGJ5KPCHHAEDYK4VIMLZD7VK");
        assert_eq!(value, 15125179076792208082_u64);
    }
    ...
    mod test_from_hex {
        ...
        fn test_should_create_from_hex_str() {
            for test_case in HEX_TEST_VECTORS {
                ...
                assert_eq!(value.to_dto(), test_case.dto);
            }
        }
    ...
    mod test_operation {
        ...
        #[test]
        fn test_should_return_subtract_value() {
            let value = Uint64::from(100);
            let other = Uint64::from(1);
            let result = value - other;
            assert_eq!(*result, 99);
