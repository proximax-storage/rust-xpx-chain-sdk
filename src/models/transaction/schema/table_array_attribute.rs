...
impl TableArrayAttribute {
    pub fn new(name: String, schema: Vec<Box<dyn SchemaAttribute>>) -> Self {
        TableArrayAttribute { abs_schema_attribute: AbstractSchemaAttribute { name }, schema }
    }
}
...
impl SchemaAttribute for TableArrayAttribute {
    fn serialize(&mut self, buffer: &mut [u8], position: usize, inner_object_position: usize) -> Vec<u8> {
        ...
        for i in 0..array_length {
            let start_array_position: usize = abs.find_object_start_position(
                inner_object_position, position, buffer,
            );
