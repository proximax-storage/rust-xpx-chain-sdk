...
impl MultisigAccountGraphInfoDto {
	pub fn compact(&self) -> anyhow::Result<Vec<MultisigAccountInfo>> {
		Ok(self.multisig_entries.iter().map(|item| item.compact().unwrap()).collect())
	}
}
...
impl MultisigAccountInfoDto {
	pub fn compact(&self) -> anyhow::Result<MultisigAccountInfo> {
...
impl TransactionDto for ModifyMultisigAccountTransactionInfoDto {
	fn compact(&self) -> anyhow::Result<Box<dyn Transaction>> {
		...
		let modifications = cosignatory_dto_vec_to_struct(dto.modifications, common.network_type);
