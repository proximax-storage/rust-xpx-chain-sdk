...
impl AccountInfoDto {
	pub(crate) fn compact(&self) -> anyhow::Result<AccountInfo> {
		let dto = &self.account;
		let address = Address::from_encoded(&dto.clone().address)?;
		let account_type = AccountLinkType::new(dto.clone().account_type);

		let mosaics: Vec<Mosaic> =
			dto.mosaics.iter().map(move |mosaic_dto| mosaic_dto.compact()).collect();

		Ok(AccountInfo {
			address,
			address_height: dto.clone().address_height.compact(),
			public_key: dto.clone().public_key,
			public_key_height: dto.public_key_height.compact(),
			account_type,
			mosaics,
		})
	}
...
impl AccountNamesDto {
	pub fn compact(&self) -> anyhow::Result<AccountName> {
		...
		if dto.property_type & AccountPropertyType::AllowAddress.value() != 0 {
			...
			let modifications: Vec<AccountPropertiesEntityTypeModification> = dto
				...
				.map(move |p| AccountPropertiesEntityTypeModification {
					modification_type: AccountPropertiesModificationType::from(p.r#type),
					transaction_type: (p.value.as_u64().unwrap() as u16).try_into().unwrap(),
				})
				.collect();
			...
			Err(anyhow!("invalid AccountPropertyType"))
		}
	}
}
...
impl<'de> Deserialize<'de> for PropertiesDto {
		...
		D: Deserializer<'de>,
	{
		...
		Ok(PropertiesDto { property_type: property.r#type, mosaic_ids, addresses, entity_types })
	}
}
