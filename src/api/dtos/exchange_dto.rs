...
impl ExchangeInfoDto {
	pub(crate) fn compact(&self, network_type: NetworkType) -> anyhow::Result<UserExchangeInfo> {
		...
		for offer_info in buy_offer_offers.into_iter() {
			...
			buy_offer.push(OfferIdInfo { mosaic_id: id.as_mosaic_id()?, offer_info });
		}
		...
		for offer_info in sell_offer_offers.into_iter() {
			let id = &offer_info.mosaic.asset_id;
			sell_offer.push(OfferIdInfo { mosaic_id: id.as_mosaic_id()?, offer_info });
		}
		...
		Ok(UserExchangeInfo { owner, offers: offers_map })
	}
}
...
impl OfferInfoDto {
	pub(crate) fn compact(&self, owner: PublicAccount) -> anyhow::Result<OfferInfo> {
		let dto = self.to_owned();
	...
	fn compact(&self) -> anyhow::Result<Box<dyn Transaction>> {
		...
		let offers: Vec<AddOffer> = dto
			...
			.map(|offer| {
				...
				AddOffer { offer: _offer, duration: offer.duration }
			})
			.collect();

		Ok(Box::new(AddExchangeOfferTransaction { common, offers }))
	}
}
		...
		let confirmations: Vec<ExchangeConfirmation> = dto
			...
			.map(|offer| {
				...
				ExchangeConfirmation {
					offer: _offer,
					owner: PublicAccount::from_public_key(&offer.owner, common.network_type)
						.unwrap(),
				}
			})
			.collect();

		Ok(Box::new(ExchangeOfferTransaction { common, confirmations }))
	}
}
	...
	fn compact(&self) -> anyhow::Result<Box<dyn Transaction>> {
		...
		let offers: Vec<RemoveOffer> = dto
			...
			.map(|offer| {
				...
				let asset_id: Box<dyn UnresolvedMosaicId> = if has_bits(id, NAMESPACE_BIT) {
					Box::new(NamespaceId::from(id))
				} else {
					Box::new(MosaicId::from(id))
				};

				RemoveOffer { r#type: OfferType::from(offer.offer_type), asset_id }
			})
			.collect();

		Ok(Box::new(RemoveExchangeOfferTransaction { common, offers }))
	}
}
