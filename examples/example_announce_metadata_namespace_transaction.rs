...
use xpx_chain_sdk::transaction::{
    AggregateTransaction, Deadline, MetadataV2NamespaceTransaction, Transaction,
};
...
async fn main() -> Result<(), xpx_chain_sdk::api::error::Error> {
    ...
    let namespace_id = NamespaceId::from_name("block_namespace")?;
    let mut metadata_txn = MetadataV2NamespaceTransaction::create(
        Deadline::default(),
